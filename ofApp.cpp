...
void ofApp::update() {
	...
	for (auto& location : base_location) {
		...
		for (int radius = 50; radius <= 300; radius += 10) {
			...
			this->setRingToMesh(this->face, this->frame, location, noise_location, radius, 50, face_color);
		}
	}
}
...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, glm::vec3 noise_location, float radius, float height, ofColor face_color) {
	...
	for (int deg = 0; deg < 360; deg += 1) {
		...
		for (auto& vertex : vertices) {
			...
			vertex = glm::vec4(vertex, 0) * rotation_y * rotation_z + glm::vec4(location, 0);
		}
